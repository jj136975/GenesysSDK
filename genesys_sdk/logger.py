...
class Logger(object):
    def __init__(self):
        ...
        # private properties
        self._logger = logging.getLogger('Genesys Light SDK')
    ...
    def trace(self, method, url, request_body, status_code, request_headers, response_headers):
        now = datetime.now()
        log_statement = LogStatement(level="trace", date=now.strftime("%m-%d-%Y, %H:%M:%S"), method=method, url=url,
                                     request_body=request_body,
                                     status_code=status_code, request_headers=request_headers,
                                     response_headers=response_headers)
        self._logger.log(LogLevel.LTrace,
                         log_statement.string(self.log_format, self.log_request_body, self.log_response_body))

    def debug(self, method, url, request_body, status_code, request_headers):
        now = datetime.now()
        log_statement = LogStatement(level="debug", date=now.strftime("%m-%d-%Y, %H:%M:%S"), method=method, url=url,
                                     request_body=request_body,
                                     status_code=status_code, request_headers=request_headers)
        self._logger.log(LogLevel.LDebug,
                         log_statement.string(self.log_format, self.log_request_body, self.log_response_body))

    def error(self, method, url, request_body, status_code, request_headers, response_body, response_headers):
        now = datetime.now()
        log_statement = LogStatement("error", now.strftime("%m-%d-%Y, %H:%M:%S"), method, url, request_body,
                                     status_code, request_headers, response_body, response_headers)
        self._logger.log(LogLevel.LError,
                         log_statement.string(self.log_format, self.log_request_body, self.log_response_body))

    def create_trace_config(self):
        """Create an aiohttp TraceConfig that logs requests through this logger."""
    ...
    async def _on_request_start(self, session, ctx, params):
        ctx.method = str(params.method)
        ctx.url = str(params.url)
        ctx.headers = dict(params.headers)

    async def _on_request_end(self, session, ctx, params):
        response = params.response
        status = response.status
        response_headers = dict(response.headers)

        if status >= 400:
            self.error(ctx.method, ctx.url, None, status, ctx.headers, None, response_headers)
        else:
            self.trace(ctx.method, ctx.url, None, status, ctx.headers, response_headers)

    async def _on_request_exception(self, session, ctx, params):
        method = getattr(ctx, 'method', 'UNKNOWN')
        url = getattr(ctx, 'url', 'UNKNOWN')
        headers = getattr(ctx, 'headers', {})
        self.error(method, url, None, 0, headers, str(params.exception), None)
...
class LogStatement(object):
    def __init__(self, level, date, method, url, request_body, status_code, request_headers, response_body=None,
